...
from utils.ps.utils.chem_utils import smi2mol, mol2smi, get_submol
from utils.ps.utils.chem_utils import cnt_atom, MAX_VALENCE
from utils.ps.utils.logger import print_log
from utils.ps.molecule import Molecule
...
def get_ring_systems(mol):
    """Return list of ring systems as sets of atom indices.

    Rings sharing >= 1 atom are merged into one system, handling fused rings
    (e.g. naphthalene → 1 system of 10 atoms) and spiro centers correctly.
    """
...
class MolInSubgraph:
    def __init__(self, mol, kekulize=False):
        self.mol = mol
        self.smi = mol2smi(mol, kekulize)
        self.kekulize = kekulize
        self.subgraphs, self.subgraphs_smis = {}, {}  # pid is the key
        ...
        # Ring systems: compound subgraphs with PIDs >= n_atoms (no collision with atom-index PIDs)
        for i, ring_system in enumerate(ring_systems):
            pid = n_atoms + i
            submol = get_submol(mol, sorted(ring_system), kekulize=self.kekulize)
            smi = mol2smi(submol, self.kekulize)
            self.subgraphs[pid] = {idx: mol.GetAtomWithIdx(idx).GetSymbol() for idx in ring_system}
            self.subgraphs_smis[pid] = smi
            for aid in ring_system:
                self.inversed_index[aid] = pid

        # Non-ring atoms: singleton subgraphs with PID = atom index
        for atom in mol.GetAtoms():
            idx, symbol = atom.GetIdx(), atom.GetSymbol()
            if idx not in ring_atom_set:
                self.subgraphs[idx] = {idx: symbol}
                self.subgraphs_smis[idx] = symbol
                self.inversed_index[idx] = idx

        self.upid_cnt = n_atoms + len(ring_systems)
        self.dirty = True
...
def graph_bpe(fname, vocab_len, vocab_path, cpus, kekulize, min_frequency):
    # load molecules
    ...
    # pre-seed ring systems into vocabulary
    print_log("Pre-seeding ring systems ...")
    ring_freq = {}
    for smi in smis:
        try:
            mol_tmp = smi2mol(smi, kekulize)
            if mol_tmp is None:
                continue
            ring_systems = get_ring_systems(mol_tmp)
            seen_in_mol = set()
            for ring_system in ring_systems:
                submol = get_submol(mol_tmp, sorted(ring_system), kekulize=kekulize)
                ring_smi = mol2smi(submol, kekulize)
                if ring_smi not in seen_in_mol:
                    seen_in_mol.add(ring_smi)
                    ring_freq.setdefault(ring_smi, 0)
                    ring_freq[ring_smi] += 1
        except Exception:
            pass
    kept_ring_cnt = 0
    for ring_smi in sorted(ring_freq, key=lambda s: ring_freq[s], reverse=True):
        if ring_freq[ring_smi] < min_frequency:
            continue
        if ring_smi not in details:
            selected_smis.append(ring_smi)
            details[ring_smi] = [cnt_atom(ring_smi), ring_freq[ring_smi]]
            kept_ring_cnt += 1
    print_log(
        f"Pre-seeded {kept_ring_cnt}/{len(ring_freq)} ring system types into vocab (min_frequency={min_frequency})"
    )
    # bpe process
    ...
    while len(selected_smis) < vocab_len:
        ...
        for smi in freqs:
            ...
            if cnt > max_cnt:
                max_cnt = cnt
                merge_smi = smi
        if max_cnt < min_frequency:
            print_log(f"Max frequency {max_cnt} of neighboring subgraphs < min_frequency {min_frequency}. Stop.")
            break
        # merge
        for mol in mols:
            mol.merge(merge_smi)
...
class Tokenizer:
    ...
    def tokenize(self, mol):
        ...
        if isinstance(mol, str):
            ...
            smiles = mol2smi(mol, self.kekulize)
        if "." in smiles:
...
def parse():
    ...
    parser.add_argument(
        "--smiles",
        type=str,
        default="COc1cc(C=NNC(=O)c2ccc(O)cc2O)ccc1OCc1ccc(Cl)cc1",
    ...
    parser.add_argument("--min_frequency", type=int, default=10, help="Minimum frequency to be included in vocab")
    parser.add_argument(
        "--kekulize",
        action="store_true",
    ...
    if args.min_frequency < 0:
        parser.error("--min_frequency must be non-negative")
    return args
...
if __name__ == "__main__":
    ...
    print(smi)
    assert mol2smi(smi2mol(smi, args.kekulize), args.kekulize) == mol2smi(
        smi2mol(args.smiles, args.kekulize), args.kekulize
    )
